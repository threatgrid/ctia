...
(defn- service-subgraph* [{:keys [get select-keys update selectors graph empty-graph]}]
...
(s/defn service-subschema :- s/Schema
  ...
  just the specified services and their optionality.
  
  (service-subschema
    {:ConfigService {:get-in-config <...>
                     (s/optional-key :get-config) <...>}
     (s/optional-key :FooService) {:f1 <...>
                                   :f2 <...>}
     :BarService {:b1 <...>}}
    :ConfigService [:get-config]
    :FooService [:f2 :f3]
    :MissingService [:m1])
  ;=> {:ConfigService {(s/optional-keys :get-config) <...>}
  ;    (s/optional-key :FooService) {:f2 <...>}}
  "
  [graph :- s/Schema & selectors]
  {:pre [(map? graph)]}
  (service-subgraph*
    {:get st/get
     :select-keys st/select-keys
     :update st/update
     :graph graph
     :selectors selectors}))
