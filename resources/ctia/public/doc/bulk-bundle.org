...
3. New entities are created in the same way as the `/bulk` API endpoint with transient IDs resolutions. If query parameter `patch-existing=true`, then existing entities are similarly patched with `result=updated`; otherwise, existing entities are not modified with `result=exists`.
