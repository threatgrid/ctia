...
| ctia.store.es.[entity].auth.params       | authentication parameter map (or string containing map) for ES, see [[https://github.com/threatgrid/ductile][ductile documentation]] | =(get-in ductile.schemas/AuthParams [:params])= |
