(ns cia.stores.es.filter-test
  (:require [cia.stores.es.filter :as f]
            [clojure.test :refer [deftest is]]))

(deftest nested-terms-test
  (is (= (f/nested-terms [[[:observable :type] "ip"]
...
(deftest mk-flat-filter-test
  (is (= (f/mk-flat-filter {:judgement "judgement-a71c737f-a3bc-428d-a644-1676e00a758d"})
         [{:terms {:judgement ["judgement-a71c737f-a3bc-428d-a644-1676e00a758d"]}}])))

(deftest filter-map->terms-query-test
  (is (= (f/filter-map->terms-query {[:observable :type] "ip"
...
(deftest indicators-by-judgements-query-test
  (is (= (f/indicators-by-judgements-query
...
(deftest unexpired-judgements-by-observable-query-test
  (is (= (f/unexpired-judgements-by-observable-query
