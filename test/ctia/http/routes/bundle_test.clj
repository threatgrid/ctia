...
(deftest valid-external-id-test
  (is (= "ctia-1"
         (sut/valid-external-id ["invalid-1" "invalid-2"  "ctia-1" "ctia-2"]))))
      ...
      (let [bundle {:type "bundle"
        ...
        (doseq [entity (concat indicators
          ...
          (validate-entity-record
           (entity-result bundle-result entity)
           entity))))
    (testing "Update"
      (let [bundle
            {:type "bundle"
             :source "source"
             :indicators (set (map update-description indicators))
             :sightings (set (map update-description sightings))
             :relationships (set (map update-description relationships))}
            response (post "ctia/bundle/import"
                           :body bundle
                           :headers {"Authorization" "45c1f5e3f05d0"})
            bundle-result (:parsed-body response)]
