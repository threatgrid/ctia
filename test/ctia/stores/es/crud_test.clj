    ...
    (helpers/fixture-ctia-with-app
     (fn [app]
       (let [conn-state (get-conn-state app :sighting)
         ...
         (let [batch-size 3
               res-default (sut/lazy-get-docs-with-indices conn-state
                                                           (map :id tested-sample)
                                                           {}
                                                           batch-size)
               nb-rec (atom 0)
               fake-get-doc-with-indices (fn [conn-state ids es-params]
                                           (swap! nb-rec inc)
                                           (sut/get-docs-with-indices conn-state ids es-params))
               res-lazyness-check (sut/lazy-get-docs-with-indices conn-state
                                                                  (map :id tested-sample)
                                                                  {}
                                                                  batch-size
                                                                  fake-get-doc-with-indices)]
           (testing "lazy-get-docs-with-indices shall return all documents matching given ids"
             (is (= (set tested-sample)
                    (set (map :_source res-default)))))
           (testing "only consumed batches shall be realized."
             (doseq [elem res-default]
               (base-check elem))
             (doall (take (dec batch-size)
                          res-lazyness-check))
             (is (= 1  @nb-rec))
             (doall (take batch-size res-lazyness-check))
             (is (= 1  @nb-rec))
             (doall (take (inc batch-size) res-lazyness-check))
             (is (= 2  @nb-rec))
             (doall (take (* 2 batch-size) res-lazyness-check))
             (is (= 2  @nb-rec))
             ;; realize and check all the collection
             (doseq [elem res-lazyness-check]
               (base-check elem))
             (is (<= (/ (count tested-sample) batch-size)
                     @nb-rec)))

           (is (= (count tested-sample)
                  (count
                   (sut/lazy-get-docs-with-indices conn-state
                                                   (map :id tested-sample)
                                                   {}
                                                   102 ;; more than default es limit
                                                   fake-get-doc-with-indices))))))))))
